...
from oarepo_ui.resources.templating.filters import value, ui_value, as_array, as_dict, empty
import pytest
...
def test_field_data(field_data_test_obj):
    ...
    assert FieldData.ui_value(title) == api_value_serialization['metadata']['title'] # should just return API value, because there is no ui representation
    ...
    assert FieldData.ui_value(publisher, format="some_non_existing_format") == api_value_serialization['metadata']['publisher']
    ...
    assert FieldData.ui_value(publication_date) == api_value_serialization['metadata']['publication_date'] # no format specified
    assert FieldData.ui_value(publication_date, format='l10n_medium') == ui_value_serialization['publication_date_l10n_medium']
    assert FieldData.ui_value(publication_date, format='l10n_long') == ui_value_serialization['publication_date_l10n_long']
    ...
    assert all(isinstance(x, FieldData) for x in creators_array), \
    "Not all items are instances of FieldData"
    
    creator0 = creators_array[0]    
    assert FieldData.value(creator0['person_or_org']['name']) == api_value_serialization['metadata']['creators'][0]['person_or_org']['name']    
    assert FieldData.value(creator0['role']['title']) == api_value_serialization['metadata']['creators'][0]['role']['title']
    assert FieldData.ui_value(creator0['role']['title']) == ui_value_serialization['creators']['creators'][0]['role']['title']
    
    creator1 = creators_array[1]
    assert FieldData.dict(creator1['person_or_org']).keys() == {"type","name","given_name","family_name"}
    assert FieldData.value(creator1['person_or_org']['name']) == api_value_serialization['metadata']['creators'][1]['person_or_org']['name']
    
    assert FieldData.ui_value(metadata["description"]) == api_value_serialization['metadata']['description'] # fallback to api value
    ...
    assert FieldData.ui_value(id_value) == api_value_serialization['metadata']['resource_type']['id'] # fallback to API value
    ...
    assert FieldData.ui_value(resource_type_title, format='l10n') == ui_value_serialization['resource_type']['title_l10n']
    assert FieldData.ui_value(resource_type_title, format="") == api_value_serialization['metadata']['resource_type']['title'] # no format, fallback to API value
    
    
    assert isinstance(record._api_data, dict)
    assert isinstance(record._ui_data, dict)
    assert isinstance(metadata['creators']['creators']._api_data, list)
    assert not isinstance(metadata['creators']['creators']._api_data[0], FieldData)
    assert metadata['title']._api_data == api_value_serialization['metadata']['title']
    assert non_existing._api_data == "sentinel: empty field data"
    assert creators_array[0]._ui_data == ui_value_serialization['creators']['creators'][0]
    ...
    assert FieldData.value(media_files['dummy_field_empty_string']) == "" # originally was empty string (not filled value)
    ...
    assert not FieldData.ui_value(media_files['enabled'])
    ...
    assert FieldData.value(media_files['entries'], default="default_value") == {} # is was empty dictionary originally, so it just return empty dict
    assert FieldData.value(media_files['entries']) == {}
    assert not bool(media_files['entries']) # it was NOT filled
    
    assert FieldData.value(media_files['order'], default="default_value") == [] # is was empty list originally, so it just return empty list
...
def test_filter_ui_value(field_data_test_obj):    
   ...
   assert ui_value(record['metadata']['publication_date'], format='l10n_long') == ui_value_serialization['publication_date_l10n_long']
